...
#pragma once
#ifndef _WDL_RESAMPLER_MOD_H_
#define _WDL_RESAMPLER_MOD_H_
...
public:
  ...
  struct FractionTop;
  void Reset(FractionTop fracpos=FractionTop(0));
  void SetRates(int rate_in, int rate_out);
#else
  ...
  int ResampleOut(WDL_ResampleSample *out, int nsamples_in, int nsamples_out, int nch);
    ...
    struct Fraction {
        ...
        Fraction Reciprocal() const { return Fraction(den, num); }
        explicit operator double() const { return (double)num / (double)den; }
        explicit operator float() const { return (float)num / (float)den; }
        explicit operator int() const { return num / den; }
        bool operator==(Fraction o) const { return num == o.num && den == o.den; }
        bool operator!=(Fraction o) const { return num != o.num || den != o.den; }
        int num = 0;
        int den = 0;
    };
    ...
    struct FractionTop {
        FractionTop() = default;
        explicit FractionTop(int num_) : num(num_) {}
        bool operator==(Fraction o) const { return num == o.num; }
        bool operator!=(Fraction o) const { return num != o.num; }
        int num = 0;
    };
#endif
...
#ifdef WDL_RESAMPLE_RATIONAL
  Fraction m_ratio WDL_FIXALIGN;
  FractionTop m_fracpos;
#else
