...
//SPDX-License-Identifier: zlib OR LGPL-2.0-or-later
//Note: This source code is a personal modification of "WDL/resample.h".
...
#ifdef WDL_RESAMPLE_RATIONAL
void WDL_Resampler::Reset(FractionTop fracpos)
#else
...
#ifdef WDL_RESAMPLE_RATIONAL
void WDL_Resampler::SetRates(int rate_in, int rate_out) 
#else
...
const WDL_SincFilterSample *WDL_Resampler::BuildLowPass(double filtpos, bool *isIdeal) // only called in sinc modes
{
  ...
  if (wantinterp)
  {
    ...
    {
#ifdef WDL_RESAMPLE_RATIONAL
      if (m_ratio.num < m_ratio.den)
#else
      if (m_ratio < 1.0)
#endif
      {
#ifdef WDL_RESAMPLE_RATIONAL
        const int irat = (int)m_ratio.Reciprocal();
        if (irat > 1 && irat*m_ratio.num == m_ratio.den) ideal_interp=irat;
#else
      ...
      {
#ifdef WDL_RESAMPLE_RATIONAL
        if (m_ratio.num % m_ratio.den == 0) ideal_interp=1; // eg 96k to 48k, only need one table
#else
...
#ifdef WDL_RESAMPLE_RATIONAL
        const int in1 = m_ratio.num, out1 = m_ratio.den;
#else
        const int in1 = (int)m_sratein, out1 = (int)m_srateout;
#endif
        if (out1 > 0 && in1 > 0
#ifndef WDL_RESAMPLE_RATIONAL
            && m_sratein == (double)in1 && m_srateout == (double)out1
#endif
            )
        {
...
#ifdef WDL_RESAMPLE_RATIONAL
      if (ideal_interp > 0 && m_fracpos.num != 0)
#else
...
{ 
#ifdef WDL_RESAMPLE_RATIONAL
  Fraction v(m_samples_in_rsinbuf-m_filtlatency, m_ratio.num);
...
#ifdef WDL_RESAMPLE_RATIONAL
    m_pre_filter->setParms((double)m_ratio.Reciprocal(),m_filterpos,m_filterq, m_prepost_filtercnt,nch);
#else
...
#ifdef WDL_RESAMPLE_RATIONAL
    m_post_filter->setParms((double)m_ratio,m_filterpos,m_filterq, m_prepost_filtercnt,nch);
#else
...
#ifdef WDL_RESAMPLE_RATIONAL
  if (!m_feedmode) sreq = (m_ratio.num * out_samples / m_ratio.den) + 4 + fsize - m_samples_in_rsinbuf;
#else
...
int WDL_Resampler::ResampleOut(WDL_ResampleSample *out, int nsamples_in, int nsamples_out, int nch)
{
...
#ifdef WDL_RESAMPLE_RATIONAL
  auto splitPosition = [](Fraction pos_, int &ipos, double &fpos)
  {
    double pos = (double)pos_;
    ipos = (int)pos;
    fpos = pos-(int)pos;
  };
  auto incrPosition = [](Fraction &a, Fraction b)
  {
      assert(a.den == b.den);
      a.num += b.num;
  };
#else
  auto splitPosition = [](double pos, int &ipos, double &fpos)
  {
    ipos = (int)pos;
    fpos = pos-(int)pos;
  };
  auto incrPosition = [](double &a, double b)
  {
      a += b;
  };
#endif
  ...
  int ret=0;
#ifdef WDL_RESAMPLE_RATIONAL
  Fraction srcpos(m_fracpos.num, m_ratio.den);
  Fraction drspos = m_ratio;
#else
...
#ifdef WDL_RESAMPLE_RATIONAL
    if (m_ratio.num > m_ratio.den) filter=BuildLowPass((double)Fraction(100*m_ratio.den, 103*m_ratio.num), &isideal);
#else
    ...
    else if (nch == 1)
    {
      if (isideal)
        while (ns--)
        {
          int ipos;
          double fracpos;
          splitPosition(srcpos, ipos, fracpos);

          if (ipos >= filtlen-1)  break; // quit decoding, not enough input samples

          SincSample1N(outptr,localin + ipos,fracpos,filter,filtsz,oversize);
          outptr ++;
          incrPosition(srcpos, drspos);
          ret++;
        }
      else
        while (ns--)
        {
          ...
          SincSample1(outptr,localin + ipos,fracpos,filter,filtsz,oversize);
          outptr ++;
          incrPosition(srcpos, drspos);
          ret++;
        }
    ...
    else if (nch==2)
    {
      if (isideal)
        while (ns--)
        {
          ...
          SincSample2N(outptr,localin + ipos*2,fracpos,filter,filtsz,oversize);
          outptr+=2;
          incrPosition(srcpos, drspos);
          ret++;
        }
        ...
        while (ns--)
        {
          ...
          SincSample2(outptr,localin + ipos*2,fracpos,filter,filtsz,oversize);
          outptr+=2;
          incrPosition(srcpos, drspos);
          ret++;
        }
    ...
    else
    {
      if (isideal)
        while (ns--)
        {
          ...
          SincSampleN(outptr,localin + ipos*nch,fracpos,nch,filter,filtsz,oversize);
          outptr += nch;
          incrPosition(srcpos, drspos);
          ret++;
        }
        ...
        while (ns--)
        {
          ...
          SincSample(outptr,localin + ipos*nch,fracpos,nch,filter,filtsz,oversize);
          outptr += nch;
          incrPosition(srcpos, drspos);
          ret++;
        }
...
#ifdef WDL_RESAMPLE_RATIONAL
    Fraction adj=Fraction(srcpos.num + srcpos.den * (-m_samples_in_rsinbuf + outlatadj), drspos.num);
    if (adj.num>0)
    {
      ret -= (int) ((adj.num + adj.den / 2) / adj.den);
      if (ret<0)ret=0;
    }
#else
...
#ifdef WDL_RESAMPLE_RATIONAL
  m_fracpos.num = srcpos.num - isrcpos * srcpos.den;
#else
